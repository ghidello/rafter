...
| External-sentinel physical symlink cleanup | Host denied symlink creation privilege; test records unavailable capability | Passed on Windows, Ubuntu, and macOS |
...
The [cross-platform cleanup CI run](https://github.com/ghidello/rafter/actions/runs/33731630863) completed successfully
on Windows, Ubuntu, and macOS. CI treats physical-link creation failure as a test failure on every matrix platform, so
the green matrix proves that the destructive cleanup removed the internal link while preserving its external sentinel.
Package integrity also passed after all three test jobs completed.
